...
class UsersWP_Forms {
	...
	public function process_register() {
		...
		$save_result = $this->save_user_extra_fields( $user_id, $result, 'register' );
			uwp_update_usermeta( $user_id, 'youtube', serialize($_POST['youtube']));
	...
	public function form_input_file( $html, $field, $value, $form_type ) {
		...
		if ( empty( $html ) ) {
			...
			<div id="<?php echo $field->htmlvar_name; ?>_row"
				...
				<?php if($field->htmlvar_name=='image_1'){ ?>
				<input name="<?php echo $field->htmlvar_name; ?>[]"
					...
					?> type="<?php echo $field->field_type; ?>" multiple>
				<?php }else{ ?>
...
//print_r($field);
		...
		if ( empty( $html ) ) {
			...
			if ( $design_style ) {
				$required = ! empty( $field->is_required ) ? ' <span class="text-danger">*</span>' : '';
if($field->htmlvar_name=='youtube'){  ?>

<button class="add_field_button">+</button>
				<?php if($form_type=='account'){
					$i=0;
					echo '<div data-argument="youtube" class="form-group btn-youtube"><label for="youtube" class="sr-only  ">YouTube url</label>';
					echo '<input type="url" name="youtube[]" id="youtube" placeholder="YouTube url" title="YouTube url" value="'.unserialize($value)[0].'" class="form-control ">';
					foreach(unserialize($value) as $val){				
						if($i>0){
							echo '<div><input type="url" name="youtube[]" id="youtube" placeholder="YouTube url" title="YouTube url" value="'.$val.'" class="form-control "><a href="#" class="remove_field">-</a></div>';
						}
