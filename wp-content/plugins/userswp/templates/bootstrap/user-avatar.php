...
if(!$uwp_in_user_loop){ ?>
	<?php 
$usermetadata = uwp_get_usermeta_row($user->ID); 

?>
<div class="card shadow-0 border-0 mw-100 bg-transparent"><?php } ?>
...
<div class="card-body  <?php if(!$uwp_in_user_loop){ ?>mt-xl-0 pt-0<?php }else{?>text-center pb-0<?php }?>">
	...
	<div class="row justify-content-center">
		...
		<div class="col <?php if($uwp_in_user_loop){?>col-5<?php }?> text-center tofront ">
			...
			<img class="gffg rounded-circle shadow border border-white border-width-4 p-0 mw-100"
			     src="<?php echo  ($usermetadata->avatar_thumb)?esc_url( $avatar_url ):(!$usermetadata->upload_a_pic)?esc_url( $avatar_url ):esc_url($usermetadata->upload_a_pic);?>" alt="<?php _e("User avatar","userswp");?>" height="<?php echo esc_attr( $size ); ?>" width="<?php echo esc_attr( $size ); ?>">
